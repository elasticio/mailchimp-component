...
function processAction(msg, conf) {
    ...
    co(function*() {
        ...
        self.logger.info('Pushing data to mailchimp payload=%j', payload);
        const hash = md5(body.email.toLowerCase());
        const response = yield mailchimp.put('/lists/' + listId + '/members/' + hash, payload);
        self.logger.info('Received response response=%j', response);
        this.emit('data', messages.newMessageWithBody(response));
        this.emit('end');
    }.bind(this)).catch(err => {
        self.logger.error('Error occured', err.stack || err);
        this.emit('error', err);
        this.emit('end');
    });
}